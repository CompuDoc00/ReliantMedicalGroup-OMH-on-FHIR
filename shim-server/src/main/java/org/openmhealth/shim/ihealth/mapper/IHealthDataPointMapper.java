...
public abstract class IHealthDataPointMapper<T extends SchemaSupport> implements DataPointMapper<T, JsonNode> {
    ...
    private static final Logger logger = getLogger(IHealthDataPointMapper.class);
    ...
    public List<DataPoint<T>> asDataPoints(List<JsonNode> responseNodes) {
        ...
        catch(MissingJsonNodeMappingException e) {
            //*LDG Now we're going to see if this was the quirk when authorizing a second iHealth device which gives this error in the node:
            //*LDG              {"BackValue":"","Client_Para":null,"Error":"unknown_token","ErrorCode":4003,"ErrorDescription":"Token is unknown"}
            logger.info("iHealth DataPoint exception occurred="+e);                         //*LDG For now we just want to see what's going on
            String errorMessage = " "+e;                                                    //*LDG A trick to convert to a string
            if (!errorMessage.contains("4003")) {                                                      //*LDG If this doesn't contain the error code we know about, we want to continue with throwing the error
                throw e;
            }
        }
        return dataPoints;
