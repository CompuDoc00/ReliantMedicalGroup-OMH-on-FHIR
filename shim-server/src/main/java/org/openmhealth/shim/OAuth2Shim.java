...
public abstract class OAuth2Shim implements Shim {
    ...
    public AuthorizationResponse processRedirect(HttpServletRequest servletRequest)
        ...
        logger.info("AuthorizationResponse processRedirect preparing to send stateKey=" + stateKey);
        ...
        logger.info("AuthorizationResponse processRedirect preparing to send authorizationCode=" + authorizationCode);
        ...
        try {
            ...
            logger.info("AuthorizationResponse processRedirect preparing to send username=" + authorizationRequestParameters.getUsername());
            logger.info("AuthorizationResponse servletRequest RequestURL is: " + servletRequest.getRequestURL());
            logger.info("AuthorizationResponse servletRequest QueryString is: " + servletRequest.getQueryString());
            logger.info("OK,now we're going to trigger a data request and potentially authentication/authorization request");
            ...
            /**
             * By this line we will have an approved access token or not
             * If we do not, then we delete the access parameters entity.
             */

            logger.info("We've completed the data request and potentially authentication/authorization request");
            logger.info("AuthorizationResponse processRedirect received AccessToken=" + restTemplate.getAccessToken());

            if (restTemplate.getAccessToken() == null) {
                logger.warn("CRAP!!! We're about to delete the "+getShimKey()+" accessParameters for " + authorizationRequestParameters.getUsername());
            ...
            logger.info("AuthorizationResponse processRedirect latest authorization status in mongo=" + AuthorizationResponse.authorized(accessParameters));
            logger.info("AuthorizationResponse.AccessParameters is: " + AuthorizationResponse.authorized(accessParameters).getAccessParameters());
            logger.info("AuthorizationResponse.Type is: " + AuthorizationResponse.authorized(accessParameters).getType());
            return AuthorizationResponse.authorized(accessParameters);
        }

        catch (OAuth2Exception e) {
            //TODO: OAuth2Exception may include other stuff
            logger.error("Problem getting or trying out the token", e);
