 ...
 */
@CrossOrigin        //*LDG Added this to enable Cross Origin Requests (CORS) to accommodate different docker container external naming conventions, etc...
@RestController
...
public class PatientDataController {
     ...
     * @param shimkey the ID for the patient in Shimmer *LDG Well, no, it's actually googlefit or fitbit.  shimmerId is the ID for the patient in Shimmer!
     * @return
     */
    ...
    public ModelAndView authenticateWithShimmer(ModelMap model,
        ...
        logger.debug("OMH-Server was called by GET " + model.toString());
        ...
        logger.debug("This is the response code from Shimmer : " + shimmerResponse.getResponseCode() );
        logger.debug("This is the OK code that we wanted from Shimmer : " + HttpStatus.OK.value() );
        ...
        //*LDG This is also a hack.  Reverse Proxy server seems to lose the Model, so we're going to sneak the ShimmerID in fron of the stateKey in the redirect URL, separated by a "."
        //Depending on which shim we're using, the state is either stored as "state=" or "state%3D" (which is URL encoded)
        redirectUrl = redirectUrl.replace("state=","state=" + userShimmerId + ".");
        redirectUrl = redirectUrl.replace("state%3D","state%3D" + userShimmerId + ".");
    ...
    public ResponseEntity findDocumentReference(@RequestParam(name="subject", required=true) String shimmerId,
        ...
        ShimmerResponse shimmerResponse = shimmerService.retrievePatientData(applicationUser, dateQueries, dataType);
    ...
    public Bundle searchBinaryBundle(@RequestHeader("Accept") String acceptHeader,
        ...
        logger.debug("Retrieving Binary with URL param");
    ...
    public ResponseEntity findObservation(@RequestParam(name="subject", required=true) String shimmerId,
        ...
        shimmerResponse = shimmerService.retrieveShimmerData(ShimmerService.SHIMMER_STEP_COUNT_RANGE_URL, applicationUser, dateQueries, dataType);
        if( shimmerResponse.getResponseCode() != HttpStatus.OK.value()){
            return ResponseEntity.status(shimmerResponse.getResponseCode()).body(shimmerResponse.getResponseData());
        }
        ...
        try {
            observations = responseService.generateObservationList(shimKey, shimmerResponse.getResponseData(), dataType);
        }
        catch(IOException ioe){
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("Could not generate observation.");
        }

        logger.debug("ObservationList successfully created.  Now bundling the observations");
    ...
    public ModelAndView handleShimmerOauthCallback(ModelMap model,
        ...
        //*LDG Since we hacked the reverse proxy to pass the shimmerId as part of the state, we need to clean it up now.
        String[] pieces = state.split("\\.");         //*LDG Split into the shimmerId and the real state. Note that the delimiter is a regular expression so you need the escape character, that too needs to be escaped!
        if (pieces.length == 2) {                    //*LDG If for some reason we only found 1 piece (pieces[0]) then we must not have inserted a shimmerId and we're done
            state = pieces[1];                       //*LDG Restore the original meaning of state
            //if (shimmerId.equals("")){                  //*LDG If we couldn't find the shimmerId from the Model, then we need to restore it. HOWEVER it appears that the prior one is saved in the model, so we need to always replace it!
                shimmerId = pieces[0];              //*LDG Actually, always load the shimmer ID as noted above
            //}
        }

        logger.debug("Usable shimmer id " + shimmerId);
        logger.debug("Usable State " + state);
        ...
        logger.debug("Authentication and Authorization successful with shimmer id " + shimmerId);
